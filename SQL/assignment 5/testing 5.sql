...
CREATE VIEW NV_SALE AS
    SELECT 
        A.AccountID, A.Email, A.Fullname, D.DepartmentName
    FROM
        `ACCOUNT` AS A
            JOIN
        `DEPARTMENT` AS D USING (DEPARTMENTID)
    WHERE
        D.DepartmentName LIKE '%SALE%';

SELECT * FROM NV_SALE;

-- QUESTION 2
DROP VIEW ACC_JOIN_GR;
CREATE OR REPLACE VIEW ACC_JOIN_GR AS
    SELECT 
        GA.AccountID,
        A.Fullname,
        A.POSITIONID,
        COUNT(GA.AccountID) AS SN
    FROM
        `ACCOUNT` AS A
            JOIN
        GROUPACCOUNT AS GA USING (ACCOUNTID)
    GROUP BY (AccountID);
...
-- QUESTION 3
create OR replace VIEW DODAI AS

SELECT QuestionID,Content,character_length(CONTENT) AS DODAI
FROM `QUESTION`
WHERE character_length(CONTENT) > '14';
